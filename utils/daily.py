...
# ВАЖНО: тройные "простые" кавычки ''' или """ — без «умных» кавычек.
_SCHEMA = """
...
# Создаём схему один раз при импорте
with _con() as con:
...
# Фасад, чтобы в коде использовать daily_store.add/list/mark_done/delete
class _DailyFacade:
    add = staticmethod(add)
    list = staticmethod(list)
    mark_done = staticmethod(mark_done)
    delete = staticmethod(delete)

daily_store = _DailyFacade()
